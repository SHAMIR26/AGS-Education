document.addEventListener('DOMContentLoaded', function() {
    ...
    if (user.role === 'Student') {
        ...
        // Fetch from backend
        fetch(`http://localhost:3000/api/mycourses?email=${encodeURIComponent(user.email)}&role=Student`)
            .then(res => res.json())
            .then(purchased => {
                if (!purchased.length) {
        ...
        fetch(`http://localhost:3000/api/mycourses?email=${encodeURIComponent(user.email)}&role=${encodeURIComponent(user.role)}`)
            .then(res => res.json())
            .then(launched => {
                if (!launched.length) {
